...
class MEXCAPI:
    ...
    async def _send_request(self, method: str, path: str, params: dict = None, signed: bool = False):
        ...
        try:
            ...
            error_details = f"APIリクエストエラー ({method} {path}): {e.status} - {e.message}"
            if params:
                error_details += f" | パラメータ: {params}"
            self.logger.error(error_details)
            self.notifier.send_discord_message(f"APIリクエストエラー: {e.status} - {e.message} for {method} {path}...エラーの詳細はログを参照してください。")
            return None
