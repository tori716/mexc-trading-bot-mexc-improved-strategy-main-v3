...
class TradingStrategy:
    ...
    async def _check_stop_loss(self, symbol: str, position: dict, current_price: float) -> bool:
        ...
        if ohlcv_15m and ohlcv_5m:
            ...
            # 5分足ボリンジャーバンドのミドルラインをエントリー方向と不利な方向にブレイクした場合
            bb_period = self.get_adjusted_param(symbol, "BB_PERIOD", self.config["BB_PERIOD"])
            df_5m["bb_mid"] = ta.volatility.bollinger_mavg(df_5m["close"], window=bb_period)
            
            if len(df_5m) >= 2:
                bb_mid_break_signal = False
                if side == "BUY" and df_5m["close"].iloc[-1] < df_5m["bb_mid"].iloc[-1] and df_5m["close"].iloc[-2] >= df_5m["bb_mid"].iloc[-2]: # ミドルラインを下抜け
                    bb_mid_break_signal = True
                elif side == "SELL" and df_5m["close"].iloc[-1] > df_5m["bb_mid"].iloc[-1] and df_5m["close"].iloc[-2] <= df_5m["bb_mid"].iloc[-2]: # ミドルラインを上抜け
                    bb_mid_break_signal = True
                
                if bb_mid_break_signal:
                    self.logger.info(f"{symbol} テクニカル損切りシグナル発生 (BBミドルラインブレイク)")
                    self.notifier.send_discord_message(f"{symbol} テクニカル損切りシグナル発生 (BBミドルラインブレイク)")
